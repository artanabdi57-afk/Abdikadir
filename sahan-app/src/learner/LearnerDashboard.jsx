...
export default function LearnerDashboard({ session, onSignOut, onNavigateTeach }) {
  ...
  return <div className="learner-shell">
    <aside className="learner-sidebar">
      ...
      <button className="learner-teach" onClick={onNavigateTeach}>Teach on Sahan â†’</button>
    ...
    <main className="learner-main">
      <header className="learner-topbar"><div className="learner-search"><input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search coursesâ€¦" /></div><div className="learner-account"><span>{initials || 'L'}</span>{displayName}</div></header>
...
function CourseCard({ course, enrolled, busy, onOpen, onEnroll }) {
  ...
  return <article className="learner-course"><button className="learner-course-main" onClick={onOpen}><div className="learner-cover">{course.cover_url ? <img src={course.cover_url} alt="" /> : null}<span>{course.category || 'Learning'}</span><b>{course.title?.[0] || 'S'}</b></div><small>{course.level} Â· {course.lesson_count || 0} lessons Â· {course.student_count || 0} learners</small><h3>{course.title}</h3><p>{course.description || 'Build useful skills with Sahan.'}</p><div className="learner-course-meta"><span>{course.rating ? `â˜… ${course.rating}` : 'No ratings yet'}{course.review_count ? ` Â· ${course.review_count} reviews` : ''}</span><strong>{price}</strong></div></button>{enrolled ? <div className="learner-enrolled">âœ“ Enrolled Â· {Math.round(Number(course.progress || 0))}% complete</div> : <button className="learner-enroll" onClick={onEnroll} disabled={busy}>{busy ? 'Enrollingâ€¦' : course.is_free ? 'Enroll free â†’' : 'View course â†’'}</button>}</article>
}
...
function CoursePanel({ course, data, enrolled, progress, busy, onClose, onEnroll, onMark }) {
  ...
  return <div className="learner-modal-backdrop" onClick={onClose}><section className="learner-modal" onClick={e => e.stopPropagation()}><button className="learner-modal-close" onClick={onClose}>Ã—</button><div className="learner-kicker">COURSE</div><h2>{course.title}</h2><p>{course.description || 'Learn practical skills with Sahan.'}</p><div className="learner-course-stats"><span>{course.category || 'Learning'}</span><span>{course.level}</span><span>{course.rating ? `â˜… ${course.rating}` : 'No ratings yet'}</span><span>{course.student_count || 0} learners</span></div>{enrolled ? <><div className="learner-progress"><div><b>{pct}% complete</b><span>{done} of {total} lessons</span></div><i><em style={{ width: `${pct}%` }} /></i></div>{data ? <div className="learner-sections">{data.sections.map(section => <div className="learner-section" key={section.id}><h3>{section.title}</h3>{data.lessons.filter(l => l.section_id === section.id).map(lesson => <label className="learner-lesson" key={lesson.id}><input type="checkbox" checked={completed.has(lesson.id)} onChange={e => onMark(lesson.id, e.target.checked)} /><span><b>{lesson.title}</b><small>{lesson.lesson_type} Â· {lesson.duration_minutes || 0} min</small></span></label>)}</div>)}</div> : <div className="learner-loading">Loading lessonsâ€¦</div>}</> : <div className="learner-enroll-panel"><strong>{course.is_free ? 'Free course' : `${course.currency || 'USD'} ${Number(course.price || 0).toFixed(2)}`}</strong><p>{course.is_free ? 'Enroll now and start learning immediately.' : 'This course requires checkout. Payment integration will be enabled before paid enrollment.'}</p><button className="learner-enroll" onClick={onEnroll} disabled={busy}>{busy ? 'Workingâ€¦' : course.is_free ? 'Enroll free â†’' : 'View course'}</button></div>}</section></div>
}
...
function Empty({ text, action }) { return <div className="learner-empty"><h3>{text}</h3>{action && <button onClick={action}>Explore courses â†’</button>}</div> }
